 ...
 * 
 */
export const handleLog = <ErrorLike extends Error, DataLike>(logger: LoggerService, successMessage: string, warningMessage: string) => (
  task: TaskEither<ErrorLike, DataLike>,
): TaskEither<ErrorLike, DataLike> => {
  return pipe(
    task,
